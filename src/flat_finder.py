...
def _clean_info_parameters(paramerter_string):
    if paramerter_string.string:
        if len(paramerter_string.string) >= 2:
...
def _extract_price_from_header(header: str) -> float:
    header_list = (
        header.replace('<span class="_tb_left">', "")
        .replace("<strong>", "")
        .replace("</strong>", "")
        .replace("</span>", "")
        .replace(".", "")
...
def _flat_satisfies_filters(flat_info: dict, flatFilters: db.UserFlatFilters) -> bool:
    ...
    if flatFilters.maxPrice and "Preis in €" in flat_info.keys():
        if flat_info["Preis in €"]:
            preis = float(
                str(flat_info["Preis in €"]).replace(".", "").replace(",", ".")
            )
...
def _checkAddressInPolygons(address: str, polygons: list) -> bool:
...
def _filter_flats_by_location(flats: List[dict], polygons: list) -> List[dict]:
    res = []
    for flat in flats:
        if "Adresse" in flat.keys():
            addr = flat["Adresse"]
            try:
                if _checkAddressInPolygons(addr, polygons):
                    res.append(flat)
            except ValueError:
                # address can not be parsed
                pass
    return res
